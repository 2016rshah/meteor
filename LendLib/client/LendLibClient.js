//if (Meteor.isClient) {
  ...
  Session.set('removing_category', false);
  Template.categories.rem_cat = function(){
    return Session.equals('removing_category', true);
  }
  ...
  Template.categories.events({
      ...
      'click #btnRemCat': function(e, t){
        Session.set('removing_category', true);
        
        Meteor.flush();
        focusText(t.find("#rem-category"));
      },
      'keyup #rem-category': function(e, t){
        if(e.which === 13)
        {
          var catVal = String(e.target.value || "");
          if(catVal){
            if(typeof lists.find({Category:catVal}).fetch()[0] !== "undefined")
            {
              var id = lists.find({Category:catVal}).fetch()[0]._id;
              lists.remove(id);
            }
            else{
      ...
      'focusout #rem-category': function(e, t){
        Session.set('removing_category', false);
      },
  ...
  Template.list.list_selected = function(){
    return ((Session.get('current_list')!==null) && (!Session.equals('curren_list', null)));
  };
